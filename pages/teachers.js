...
export default function TeacherDashboard() {
    ...
    const [profile, setProfile] = useState(null); // Teacher profile
    const [schedule, setSchedule] = useState([]); // Weekly view with exceptions
    const [baseSchedule, setBaseSchedule] = useState([]); // Permanent Base Schedule
    const [loading, setLoading] = useState(true);
    const alertedClasses = useRef(new Set()); // To prevent duplicate 3-hour alerts
    ...
    // Notice: Teachers need to specify semester and section for each class
    ...
    useEffect(() => {
        ...
        supabase.auth.getSession().then(({ data: { session } }) => {
            ...
            else window.location.href = '/tlogin';
        });
    }, []);
        ...
        const interval = setInterval(() => {
            ...
            schedule.forEach(cls => {
                ...
                if (cls.day === currentDay) {
                    ...
                    // If class is exactly within the 3-hour window (180 mins) and hasn't been alerted/acted upon
                    if (diff <= 180 && diff > 178 && !cls.isConfirmed && !cls.isCancelled && !cls.isRescheduled) {
                        if (!alertedClasses.current.has(cls.id)) {
                            ...
                            // Save to DB so CRs/Students might also see a pending status if you build that feature
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'notifications' }, (payload) => {
                ...
                // Check if the notification concerns any of the teacher's current schedule courses and sections
    ...
    const handleConfirmClass = async (classId, courseName, section) => {
        ...
        if (!error) {
            await supabase.from('notifications').insert([{ 
                message: `✅ Confirmed: ${courseName} for Section ${section} will be held as scheduled today.` 
            }]);
        }
    ...
    const handleCancelClass = async (classId, courseName, section) => {
        ...
        if (!error) {
            await supabase.from('notifications').insert([{ 
                message: `🚨 Cancelled: ${courseName} for Section ${section} has been cancelled by ${profile.name}.` 
            }]);
        }
        fetchProfileAndSchedule(session.user.id);
    };
    ...
    const submitReschedule = async (e) => {
        ...
        if (!error) {
            await supabase.from('notifications').insert([{ 
                message: `🕒 Rescheduled: ${editingClass.course} for Section ${editingClass.section} moved to Room ${newRoom} (${newStartTime} - ${newEndTime}).` 
            }]);
    ...
    const openBaseModal = (cls = null) => {
        if (cls) {
            ...
            setIsManualSemester(!availableSemesters.includes(cls.semester));
            setIsManualSection(!availableSections.includes(cls.section));
        } else {
    ...
    const submitBaseSchedule = async (e) => {
        ...
        const payload = { 
            course: baseForm.course, 
            teacher: profile.name, // Force lock to logged in teacher
            room: baseForm.room, 
            day: baseForm.day, 
            start_time: baseForm.start_time, 
            end_time: baseForm.end_time, 
            semester: baseForm.semester, 
            section: baseForm.section 
        };
    ...
    return (
        <div style={{ background: '#f0f2f5', minHeight: '100vh', fontFamily: "'Roboto', sans-serif" }}>
            ...
            <header style={{ background: '#002147', color: '#F2A900', padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
                <div style={{ fontWeight: '900', fontSize: '1.2rem' }}>👨‍🏫 Teacher Dashboard</div>
            ...
            <div style={{ maxWidth: '1000px', margin: '20px auto', padding: '0 15px' }}>
                ...
                {activeTab === 'weekly' && (
                    <div>
                        ...
                        {schedule.length === 0 ? (
                            <p>No classes found assigned to you.</p>
                        ) : (
                            schedule.sort((a, b) => a.day.localeCompare(b.day)).map((cls) => (
                                <div key={cls.id} style={{ 
                                    ...
                                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                                        {cls.isCancelled ? (
                                            <button onClick={() => handleUndoException(cls.id, 'cancelled', cls.course, cls.section)} style={btnStyle('#6c757d')}>↩️ Undo Cancellation</button>
                                        ) : cls.isConfirmed ? (
                                            <button onClick={() => handleUndoException(cls.id, 'confirmed', cls.course, cls.section)} style={btnStyle('#6c757d')}>↩️ Mark Not Confirm</button>
                                        ) : cls.isRescheduled ? (
                                            <button onClick={() => handleUndoException(cls.id, 'rescheduled', cls.course, cls.section)} style={btnStyle('#6c757d')}>↩️ Undo Reschedule</button>
                                        ) : (
                                            <>
                ...
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: '15px', boxSizing: 'border-box', overflowY: 'auto' }}>
                    <div style={{ background: 'white', padding: '25px', borderRadius: '10px', width: '100%', maxWidth: '400px', maxHeight: '90vh', overflowY: 'auto' }}>
                        ...
                        <form onSubmit={submitBaseSchedule} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                            ...
                            {/* SEMESTER DROPDOWN */}
                            {isManualSemester ? (
                                <input type="text" placeholder="Type Semester (e.g. 1st)..." required value={baseForm.semester} onChange={(e) => setBaseForm({...baseForm, semester: e.target.value})} style={inputStyle} />
                            ) : (
                                <select required value={baseForm.semester} onChange={(e) => {
                                    if (e.target.value === 'MANUAL') { setIsManualSemester(true); setBaseForm({...baseForm, semester: ''}); }
                                    else setBaseForm({...baseForm, semester: e.target.value});
                                }} style={inputStyle}>
                            ...
                            {/* SECTION DROPDOWN */}
                            {isManualSection ? (
                                <input type="text" placeholder="Type Section (e.g. A)..." required value={baseForm.section} onChange={(e) => setBaseForm({...baseForm, section: e.target.value})} style={inputStyle} />
                            ) : (
                                <select required value={baseForm.section} onChange={(e) => {
                                    if (e.target.value === 'MANUAL') { setIsManualSection(true); setBaseForm({...baseForm, section: ''}); }
                                    else setBaseForm({...baseForm, section: e.target.value});
                                }} style={inputStyle}>
